...
4. Each Run All creates a new timestamped folder in `llm_runs/` plus a ZIP. With fixed seeds, my two reruns reproduced identical losses, updates and eval results.
5. Embedding viewer: open `embedding-viewer.html` locally and load a run's `checkpoint.json`.
6. Tests: `python -m unittest test_language_evals test_corpus`.
...
| `custom_llm_*.executed.ipynb` | Executed notebooks for both experiments, outputs intact |
...
| `llm_runs/` | Both complete run folders and their results ZIPs |
